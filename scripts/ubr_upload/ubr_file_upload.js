...
function startUpload(upload_id, debug_upload){
  ...
  // Remove the action element from the form as it is obstructing access to the form.action property!
	var elem = document.getElementsByName('action')[0];
	var old;
	if (elem != null)
	{
  ...
  if (old != null)
  {
    // Readd the action element if it was defined
