...
if($read_status->active && $read_status->bytes_uploaded < $_GET['total_upload_size']){
	...
	if($DEBUG_AJAX){
		if(!$read_status->active && $read_status->is_dir_error){
			showDebugMessage("<font color='green'>WARNING</font>: Cannot find upload temp directory " . $TEMP_DIR . $UPLOAD_ID . ".dir");
		}
		elseif(!$read_status->active && $read_status->open_dir_error){
			showDebugMessage("<font color='green'>WARNING</font>: Cannot open upload temp directory " . $TEMP_DIR . $UPLOAD_ID . ".dir");
		}
	}
}
